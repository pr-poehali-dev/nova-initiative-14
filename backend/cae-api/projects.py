...
# Альфа-тест: лимит фактически снят (как и обещает UI «∞»), но оставлен
# высокий потолок как защита от спама/ботов до выхода платных тарифов.
    ...
    with conn.cursor() as cur:
        ...
        if active_count >= DEFAULT_PROJECT_QUOTA:
            return json_response(403, {
                ...
                'message': (
                    'Достигнут технический потолок активных проектов. '
                    'Заархивируйте или удалите ненужные в списке проектов, '
                    'чтобы создать новый.'
                ),
