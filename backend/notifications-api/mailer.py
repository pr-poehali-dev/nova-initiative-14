"""
SMTP-отправка через Яндекс 360 (те же секреты, что и SSO: SSO_SMTP_USER /
SSO_SMTP_PASSWORD). Используется для письма «ваша проблема исправлена».

Логика отправки 1:1 повторяет backend/sso-auth/mailer.py — бэкенд-функции
изолированы и не могут импортировать друг друга, поэтому копия минимальна.
"""
import os
import smtplib
import ssl
...
def send_email(to: str, subject: str, html_body: str, text_body: str) -> bool:
    ...
    if not smtp_login_raw or not smtp_password:
        print('[notifications-api] SMTP secrets not configured, email skipped', flush=True)
        return False
    ...
    try:
        ...
        print(f'[notifications-api] email sent OK: to={recipient!r}', flush=True)
        return True
    except Exception as e:
        print(f'[notifications-api] SMTP error: {type(e).__name__} {e!r}', flush=True)
        return False


def fix_notification_html(greet: str, title: str, body: str, link_url: str) -> str:
    """Письмо «проблема, с которой вы столкнулись, исправлена»."""
    btn = ''
    if link_url:
        btn = f"""<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin:24px auto;">
<tr><td style="background:#c0392b;border:2px solid #c0392b;">
<a href="{link_url}" target="_blank" style="display:inline-block;padding:14px 28px;font-family:'Courier New',monospace;font-size:13px;letter-spacing:0.15em;text-transform:uppercase;color:#ffffff;text-decoration:none;font-weight:700;">Перейти на сайт</a>
</td></tr></table>"""
...
<p style="margin:0 0 18px;font-family:'Courier New',monospace;font-size:11px;letter-spacing:0.2em;text-transform:uppercase;color:#3a3a5e;">Диплом-Инж.рф · Уведомление</p>
<h1 style="margin:0 0 24px;font-size:22px;font-weight:700;color:#1a1a2e;border-bottom:2px solid #1a1a2e;padding-bottom:14px;">{title}</h1>
<p style="font-size:15px;line-height:1.55;margin:0 0 14px;">{greet}</p>
<p style="font-size:15px;line-height:1.55;margin:0 0 22px;white-space:pre-wrap;">{body}</p>
{btn}
