...
export const PROGRAM_MODULES: ProgramModule[] = [
  {
    ...
    tasks: [
      "Анализируете формулировку темы: соответствие специальности, наличие объекта и предмета проектирования",
      "Заполняете бланк задания (под нашим контролем) — цель, задачи, исходные данные, ожидаемые результаты",
      "Составляете пошаговый план с дедлайнами по каждому разделу ПЗ и чертежам",
      "Определяете перечень необходимой конструкторской документации",
    ],
    mistakes: [
  ...
  },
  {
    ...
    tasks: [
      "Подбираете источники: учебники, статьи, патенты, каталоги оборудования и материалов",
      "Выстраиваете логику обзора — от общего к частному, от проблемы к решению",
      "Анализируете существующие конструкции-аналоги, сравниваете параметры в таблице",
      "Формулируете выводы обзора и обоснование выбранного технического решения",
    ],
    mistakes: [
  ...
  },
  {
    ...
    tasks: [
      "Разрабатываете компоновочную схему изделия / узла / приспособления",
      "Обосновываете выбор материалов: марка стали, термообработка, покрытие — со ссылками на ГОСТ",
      "Выбираете стандартные комплектующие: подшипники, крепёж, уплотнения — по каталогам",
      "Прорабатываете конструктивные элементы: посадки, допуски, шероховатости",
    ],
    mistakes: [
  ...
  },
  {
    ...
    tasks: [
      "Определяете перечень расчётов: прочностные, кинематические, энергетические, тепловые",
      "Выбираете методику: расчётную схему, граничные условия, коэффициенты запаса — мы проверяем корректность",
      "Контролируете размерность — единицы СИ, согласованность между формулами",
      "Оформляете расчёты по стандарту: номер формулы, расшифровка обозначений, результат с единицами",
    ],
    mistakes: [
  ...
  },
  {
    ...
    tasks: [
      "Выполняете сборочный чертёж: разрезы, сечения, позиции, размеры сопряжений, технические требования",
      "Выполняете рабочие чертежи деталей: размерные цепи, допуски, шероховатости, базы",
      "Заполняете спецификацию по ГОСТ 2.106: разделы, обозначения, форматы",
      "Оформляете основную надпись (штамп), масштаб, форматы листов по ГОСТ 2.301",
    ],
    mistakes: [
  ...
  },
  {
    ...
    tasks: [
      "Анализируете технологичность конструкции: доступность обработки, базирование, жёсткость",
      "Составляете маршрут обработки: заготовка → операции → контроль",
      "Выбираете станки, приспособления, режущий и мерительный инструмент",
      "Рассчитываете режимы резания и нормы времени для 2–3 операций",
    ],
    mistakes: [
  ...
  },
  {
    ...
    tasks: [
      "Рассчитываете себестоимость изготовления детали / узла: материалы, зарплата, накладные",
      "Оцениваете экономический эффект от внедрения проектного решения",
      "Описываете мероприятия по охране труда: вредные факторы, СИЗ, требования к рабочему месту",
      "Оформляете расчёты в таблицы с указанием источников нормативов",
    ],
    mistakes: [
  ...
  },
  {
    ...
    tasks: [
      "Выстраиваете структуру ПЗ: титульный лист, задание, содержание, введение, главы, заключение, список литературы",
      "Выверяете оформление: шрифт, поля, нумерация страниц, заголовки, подписи к рисункам и таблицам",
      "Проверяете перекрёстные ссылки: формулы, таблицы, рисунки, приложения, литература",
      "Приводите список литературы к ГОСТ 7.1, проверяете наличие ссылок в тексте",
    ],
    mistakes: [
  ...
  },
  {
    ...
    tasks: [
      "Структурируете доклад: проблема → цель → решение → результаты → выводы",
      "Создаёте презентацию: ключевые чертежи, схемы, таблицы результатов — минимум текста",
      "Репетируете выступление: тайминг, интонация, переходы между слайдами — мы даём обратную связь",
      "Разбираете с наставником типовые вопросы комиссии и готовите лаконичные ответы",
    ],
    mistakes: [
  ...
  },
  {
    ...
    tasks: [
      "Определяете цель моделирования: какую гипотезу проверяете, какой результат нужен",
      "Задаёте постановку: граничные условия, нагрузки, свойства материала, тип сетки — мы проверяем корректность",
      "Анализируете результаты: распределение напряжений, деформации, запас прочности",
      "Сопоставляете КЭМ-расчёт с аналитическим — фиксируете расхождение и причины",
    ],
    mistakes: [
