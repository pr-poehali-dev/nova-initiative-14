...
/**
 * Шаги туториала зависят от устройства:
 *  - на ПК упоминаются горячие клавиши (N/E/S/F5) и мышь (колесо, Shift+ЛКМ);
 *  - на смартфоне горячих клавиш нет — описаны касания и жесты (щипок, два пальца).
 * Это решает проблему: на телефоне инструкции про клавиши вводили в заблуждение.
 */
function buildSteps(isMobile: boolean): TutorialStep[] {
  return [
    {
      target: "tools",
      title: "Шаг 1 из 6 · Инструменты",
      body: isMobile
        ? "Слева — три режима работы: узел, балка, выбор. Активный режим подсвечен. Переключайтесь между ними касанием нужной кнопки."
    ...
    },
    {
      ...
      body: isMobile
        ? "В режиме «Узел» коснитесь холста 2–3 раза, чтобы создать узлы — они привязываются к сетке. Щипок двумя пальцами — масштаб, перетаскивание двумя пальцами — перемещение по холсту."
    ...
    },
    {
      ...
      body: isMobile
        ? "Выберите «Балку», затем коснитесь двух узлов подряд — между ними появится элемент с материалом и сечением по умолчанию."
    ...
    },
    {
      ...
      body: isMobile
        ? "В режиме «Выбор» коснитесь любого узла. Откроется панель свойств: выберите тип опоры (защемление, шарнир) или задайте силу. Без опор расчёт не запустится."
    ...
    },
    {
      ...
      body: isMobile
        ? "Когда в панели «Проблемы модели» нет красных ошибок, нажмите «Посчитать». Если кнопка серая — посмотрите, какие проблемы перечислены."
