  ...
  phases: [
    ...
    },
    {
      ...
      tasks: [
        ...
        { title: "Единая стилистика с 2D-редактором", status: "done", note: "3D-сцена читает палитру из CSS-переменных темы: фон, узлы, стержни, опоры, реакции, сетка, ViewCube. Работает в тёмной теме; добавлены подписи выбранных узлов." },
        { title: "Создание мышью в 3D (как в 2D)", status: "done", note: "Режим «Узел» — клик по полу сцены добавляет узел (привязка к сетке); режим «Балка» — клик по двум узлам соединяет стержнем. Ползунки приведены к чертёжному стилю." },
        { title: "Подсветка точки под курсором (3D)", status: "done", note: "В режиме «Узел» под курсором — красный маркер привязки к сетке + координаты, видно куда встанет узел." },
        { title: "Полноэкранный режим канвы", status: "done", note: "Кнопка «на весь экран» в тулбаре канвы (Fullscreen API), тикет #55." },
        { title: "Fullscreen-макет 3D: канва во всю ширину + скрываемые панели", status: "done", note: "3D-редактор: канва на весь экран, панели инструментов и инспектор — плавающие оверлеи, сворачиваются язычками (референс Bambu Studio). 2D пока в прежнем макете." },
        { title: "Модальное окно ввода нагрузок", status: "done", note: "Удобная модалка ввода узловых нагрузок (Fx/Fy/Fz, Mx/My/Mz) по заметной плавающей кнопке при выборе узла." },
      ],
    },
    {
      key: "input-3d",
      ...
      horizon: "Почти готово",
      summary: "Панель свойств в 3D позволяет задать пространственную конструкцию: Z-координата узла, 6 DOF опор, узловые нагрузки по всем осям и распределённая qy/qz. Остался опциональный ручной поворот сечения.",
      tasks: [
        ...
        { title: "Распределённая нагрузка qz по второй плоскости", status: "done", note: "В панели элемента в 3D появилось поле q_z (локальная z); бэк уже считал qz — теперь его можно задать в UI." },
