...
/**
 * Готовый «продающий» текст приглашения для пересылки другу.
 * Ссылка подставляется в человекочитаемом виде (без punycode).
 */
export function buildReferralMessage(refCode: string | null | undefined): string {
  const url = buildReferralUrl(refCode);
  return (
    `Привет! Помнишь, сколько возни с расчётами балок и рам в APM и ANSYS? ` +
    `Я запустил облачный сопромат прямо в браузере — без установки и тяжёлых лицензий.\n\n` +
    `Строишь расчётную схему на экране, сервер считает и выдаёт готовые эпюры N, Q, M, ` +
    `напряжения по Мизесу + оформляет PDF-отчёт по ЕСКД (хоть в ВКР, хоть в КБ, хоть на экспертизу).\n\n` +
    `Вся база ГОСТ-профилей на месте, считает 2D-рамы за секунды. ` +
    `Открывается с любого устройства — удобно для быстрых проверок где угодно. ` +
    `Сейчас идёт альфа-тест, поэтому всё полностью бесплатно и без лимитов. ` +
    `За активный тест ещё и баллы дают на будущее.\n\n` +
    `Попробуй и поделись со своими:\n👉 ${url}`
  );
}
