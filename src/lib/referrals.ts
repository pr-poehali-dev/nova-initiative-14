...
export function buildReferralMessage(refCode: string | null | undefined): string {
  ...
  return (
    `Привет! Помнишь геморрой с расчётами балок в APM и ANSYS? ` +
    `Наткнулся на крутую тему — парни запустили облачный сопромат прямо в браузере.\n\n` +
    `Работает даже с телефона прямо на стройке или парах — ничего скачивать не надо. ` +
    `Строишь схему на экране, сервер считает и выдаёт готовые эпюры N, Q, M, напряжения по Мизесу ` +
    `+ оформляет PDF-отчёт по ЕСКД (хоть в ВКР, хоть на экспертизу).\n\n` +
    ...
    `Тестируй сам со смартфона или ноута и кидай своим:\n👉 ${url}`
  );
}
