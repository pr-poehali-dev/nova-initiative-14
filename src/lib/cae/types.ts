...
export interface ModelElement {
  ...
  label?: string;
  /**
   * Опорный вектор ориентации сечения в 3D (определяет, как повёрнуто сечение
   * вокруг оси стержня). Локальная ось z = normalize(x_local × ref_vector).
   * Если не задан — при расчёте подставляется автоматически так, чтобы изгиб в
   * плоскости xy шёл по сильной оси I_z (как в 2D). Для 2D игнорируется.
   */
  ref_vector?: [number, number, number];
}
...
/**
 * Инженерная школа (направление проектирования). Определяет соглашение
 * построения эпюры изгибающего момента — главное расхождение между
 * строительной и машиностроительной традицией:
 *  - construction: эпюра M строится со стороны РАСТЯНУТОГО волокна
 *    (строительная механика, СНиП/СП). Ось ординат вниз для балки под нагрузкой.
 *  - mechanical:   эпюра M строится по знаку величины (математическое
 *    соглашение, положительный момент — вверх). Принято в курсах сопромата
 *    и деталей машин.
 * Влияет ТОЛЬКО на сторону/знак отрисовки эпюр (на канве и в PDF),
 * сами расчётные значения усилий не меняются.
 */
export type DisciplineKind = "mechanical" | "construction";

export interface AnalysisSettings {
  /**
   * Инженерная школа — машиностроение или строительство. Определяет соглашение
   * построения эпюры момента (сторона/знак). По умолчанию mechanical.
   */
